...
# Do radius of city
# top 10 cities and cities in top 50 in MD/NY

num_cities = 10
cities_top = pd.read_csv(cities_path)\
            .sort_values('population',ascending=False)[:50]
cities_top = pd.concat([cities_top.iloc[0:10], cities_top[cities_top.state_id.isin(['MD','NY'])]])
cities_top = gpd.GeoDataFrame(
        cities_top,
        geometry=gpd.points_from_xy(cities_top.lng, cities_top.lat))


# Part 2: get a list of all the child indexes
children = []
print("Tiling...")
for feature in tqdm(cities_top.buffer(0.23).__geo_interface__['features']):
    # get radius
    children = children + list(h3.polyfill(feature['geometry'], H3_RES, True))
...
print("Getting geo boundaries of h3 tiles...")
